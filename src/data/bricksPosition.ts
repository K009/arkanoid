...
function getLevelData(canvas: HTMLCanvasElement, brick: Brick, index: number) {
   const brickAttribs1: positionsInterface[]= [];
  ...
  const levelOne = {
    ...
    levelTwo = { dx: 3, dy: -1.75, brickAttribs: brickAttribs },
    levelThree = { dx: -2, dy: -2.75, brickAttribs: brickAttribs },
    levelFour = { dx: 3, dy: -1.75, brickAttribs: brickAttribs },
    levelFive = { dx: 3, dy: -2, brickAttribs: brickAttribs };
  ...
  let brickAttribs: [positionsInterface[]] = [[brickAttribs1], [], [], [], []];
  ...
  if (this.index === 1) return levelOne;
  else if (this.index === 2) return levelTwo;
